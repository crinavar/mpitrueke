...
int main(int argc, char **argv){
    ...
    if(rank == 0){printf("MPI Proc: %i of %i\n", rank, numprocs);}
	...
	if(rank == 0){printf("ok: total time %.2f secs\n", atime);}
	...
	for(int i = 0; i < s.realizations; i++){
		if(rank == 0) {printf("[realization %i of %i]\n", i+1, s.realizations); fflush(stdout);}
		/* try a new seed */
		newseed(&s);
...
#ifdef MEASURE
	/* reduce physical results on rank 0*/
    obset_t *avt;
    if(rank == 0) {
        avt = (obset*)malloc(sizeof(obset)*s.R);
    }
    make_realization_statistics(&s);
    for(int i=0; i<s.R; ++i){
        for(int j=0; j<NUM_PHYSICAL_VALUES; j++){
            MPI_Reduce(&(s.obstable[i].rdata[j].mean), &(avt[i].rdata[j].mean), 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
            MPI_Reduce(&(s.obstable[i].rdata[j].errbar), &(avt[i].rdata[j].errbar), 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
            if(rank == 0){
                s.obstable[i].rdata[j].mean = avt[i].rdata[j].mean/numprocs;
                s.obstable[i].rdata[j].errbar = (avt[i].rdata[j].errbar/numprocs)/sqrt(numprocs);
            }
        }
    }
    if(rank == 0){
        write_physical_results(&s);
    }
#endif
	/* total time */
	sdkStopTimer(&(s.gtimer));
    stime = sdkGetTimerValue(&(s.gtimer))/1000.0f;
	if(rank == 0){printf("ok: total time: %.2f (adapt) + %.2f (sim) secs = %.2f secs\n", atime, stime, atime + stime);}
